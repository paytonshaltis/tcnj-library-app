...
export class ComputersPage {
  ...
  lowerLevelComputers() {
    ...
    for(let i = 0; i < 4; i++) {
      ...
      if(this.basementComps[index].status == 0 ){
        ...
        ctx.beginPath();
        ctx.moveTo(745, xCoord);
        ctx.lineTo((745 + this.square_size),(xCoord + this.square_size));
        ctx.moveTo(745,(xCoord + this.square_size));
        ctx.lineTo((745 + this.square_size), xCoord);
        ctx.stroke();
        ctx.closePath();
  ...
  */
  fourthLevelComputers() {
    ...
    for(let i = 0; i < 2; i++) {
      for(let j = 0; j < 2; j++) {
        if(this.fourthFloorComps[index].status == 0 ){
           ctx.lineWidth = 8;
           ctx.strokeStyle = this.colors[this.fourthFloorComps[index].status];
           ...
           ctx.strokeStyle = 'black';
           ctx.strokeRect(yCoord, xCoord, this.square_size, this.square_size);
           ctx.fillStyle = this.colors[this.fourthFloorComps[index].status];
           ctx.closePath();
           ctx.fillRect(yCoord, xCoord, this.square_size, this.square_size);
         }
         xCoord -= 40;
